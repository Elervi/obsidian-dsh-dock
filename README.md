...
> 官方原生：直接运行官方 `dsh CLI`、官方 UI 原样嵌入——**原汁原味，一个字节不改**，
> 美化只作用于插件外壳。

## ✨ 特性

- 📦 **开箱即用** — 无需构建，复制仓库根 3 个文件即装
- 🪟 **官方原生** — 原汁原味：定位 `dsh` → spawn 官方 `dsh web` → iframe 原样嵌入，官方 UI 一个字节不改
...
- 会话全文搜索需 Node ≥ 22.5；
- 卸载/停用时 SIGTERM 关停服务；Obsidian 崩溃可能残留孤儿进程（重启后自动挂接回原端口）。
...
| 日期 | 更新 |
|---|---|
| 2026-08-19 | 「零自研」表述改为「官方原生·原汁原味」：README / 设置页 / manifest 统一为「直接运行官方 dsh CLI、官方 UI 原样嵌入，一个字节不改」 |
