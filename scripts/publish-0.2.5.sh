#!/usr/bin/env bash
# publish-0.2.5.sh - publish DSH Dock 0.2.5 via api.github.com (works even when github.com is blocked)
#
...
#
# Steps:
#   1) Push versions.json (with 0.2.5) to main via the Contents API (no git push needed)
#   2) Publish the 0.2.5 draft release if it exists; otherwise create it and upload
#      main.js / manifest.json / styles.css (built from the local working tree)
#   3) Verify latest release == 0.2.5 with all assets present
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "==> 1/3 update versions.json on main (add 0.2.5)"
